...
class Parser
{
        ...
        foreach ($chunkInfos as $chunkId => $chunkSize) {
            switch ($chunkId) {
                ...
                case 0x03043007:
                    ...
                    break;
                case 0x03043008:
                    $properties = array_merge($properties, self::parseAuthorInfo($fileHandler));
                    break;
                default:
    ...
    protected static function parseAuthorInfo($fp)
    {
        self::fetchLong($fp);
        self::fetchLong($fp);
        $properties = [];
        $properties['c8_authorLogin'] = self::fetchString($fp);
        $properties['c8_authorName'] = self::fetchString($fp);
        $properties['c8_authorZone'] = self::fetchString($fp);
        return $properties;
    }
}
